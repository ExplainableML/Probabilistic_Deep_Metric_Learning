...
class Criterion(torch.nn.Module):
    def __init__(self, opt, embeds=None, targets=None):
        ...
        self.kappa = torch.nn.Linear(in_features=self.embed_dim, out_features=self.num_proxies, bias=False)
        nn.init.constant_(self.kappa.weight, opt.loss_proxyvmf_concentration)
        self.class_idxs  = torch.arange(self.num_proxies).to(opt.device)

        self.temp = torch.nn.Parameter(torch.ones(1) * opt.loss_proxyvmf_temp, requires_grad=opt.loss_proxyvmf_learnable_temp)

        self.name = 'proxyvmf'

        self.optim_dict_list = [
            {'params': self.proxies.weight_v, 'lr': opt.lr * opt.loss_proxyvmf_proxylrmulti},
            {'params': self.kappa.weight, 'lr': opt.lr * opt.loss_proxyvmf_conclrmulti},
        ...
        self.oproxy_mode = opt.loss_oproxy_mode
        self.oproxy_dim = int(opt.loss_oproxy_mode == "nca")
        self.guidance_w = opt.loss_proxyvmf_guidance_w
    ...
    def forward(self, batch, labels, **kwargs):
        ...
        if self.subsample_p < 1:
            ...
            proxy_normed = torch.cat([self.proxies.weight_v[unique_labels], self.proxies.weight_v[neg_proxy_idxs]], dim=0)
            kappa_pos = torch.maximum(self.kappa.weight, torch.ones(1).to(batch.device) * 0.1)
        ...
        #Calculate vmfs for the images:
        batch_v = torch.nn.functional.normalize(batch, dim=1)
        batch_g = torch.norm(batch, dim=1)

        # Compute Proxy PPK similarities
        # Subtract constant from sim_to_proxies
        sim_to_proxies = torch.stack([
            log_ppk_vmf_vec(
                ...
                p=batch.shape[-1],
                mode=self.pars.loss_proxyvmf_mode
            )
            for i in torch.arange(batch_v.shape[0])
        ])

        # Compute final proxy-based NCA loss (PNCA++ formulation)
        ...
        # Guidance Objective:
        if self.guidance_w:
            guidance_loss = self.oproxy(batch_v, labels)
            loss = loss + self.guidance_w * guidance_loss

        return loss


    def oproxy(self, batch, labels):
        labels = labels.to(batch.device)
        same_labels = (labels.view(1, -1) == labels.view(-1,1))
        diff_labels = (labels.view(-1, 1) != self.class_idxs.unsqueeze(0))

        proxies = torch.nn.functional.normalize(self.proxies.weight_v, dim=-1)

        ###
        pos_sims = batch.mm(proxies[labels].T)
        sims = batch.mm(proxies.T)

        ###
        w_pos_sims  = -self.oproxy_pars['pos_alpha'] * (pos_sims - self.oproxy_pars['pos_delta'])
        w_neg_sims  =  self.oproxy_pars['neg_alpha'] * (sims - self.oproxy_pars['neg_delta'])

        pos_s = self.masked_bindev_logsumexp(
            w_pos_sims,
            mask=same_labels.type(torch.bool),
            dim=self.oproxy_dim
        )
        neg_s = self.masked_bindev_logsumexp(
            w_neg_sims,
            mask=diff_labels.type(torch.bool),
            dim=self.oproxy_dim
        )
