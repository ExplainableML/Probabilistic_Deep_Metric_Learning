...
from utilities.misc import approx_log_cp
...
class Criterion(torch.nn.Module):
    ...
    def log_sim(self, mu1, kappa1, mu2, kappa2, rho=0.5, p=512, temp=0.01):
        ...
        x = torch.nn.functional.normalize(mu1 * kappa2, dim=1)
        sim = torch.sum(x * kappa2 * mu2, dim=1)
        norm_mu2 = torch.norm(mu2 * kappa2, dim=1)
        norm_const = approx_log_cp(norm_mu2)
        detterm = torch.sum(torch.log(kappa2), dim=1) - torch.log(norm_mu2)
        logl = sim + norm_const + detterm
        logl = logl / temp
