...
class Criterion(torch.nn.Module):
    ...
    def log_sim(self, mu1, kappa1, mu2, kappa2, rho=0.5, p=512, temp=0.01):
        # We use the analytical solution here.
        # If you want the sampling based one, replace log_sum() by utilities.misc.log_ppk_vmf_vec()
        if mu1.dim() == 1:
            mu1 = mu1.unsqueeze(0)
        ...
        ppk = rho * (approx_log_cp(kappa1, p) + approx_log_cp(kappa2, p)) - approx_log_cp(rho * kappa3, p)
        ppk = ppk / temp
        return ppk.squeeze(ppk.dim() - 1)
