...
class Criterion(torch.nn.Module):
    ...
    def log_sim(self, mu1, kappa1, mu2, kappa2, rho=0.5, p=512, temp=0.01):
        ...
        cos_dist = torch.nn.functional.cosine_similarity(mu1, mu2).unsqueeze(1)

        kl = approx_log_cp(kappa1) - approx_log_cp(kappa2) + kappa1 - kappa2 * cos_dist
        kl = kl / temp

        return kl.squeeze(kl.dim() - 1)
