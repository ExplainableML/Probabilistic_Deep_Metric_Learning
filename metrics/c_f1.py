...
class Metric():
    ...
    def __call__(self, target_labels, computed_cluster_labels_cosine, features_cosine, centroids_cosine):
        ...
        for i in range(len(features_cosine)):
            ...
            # Compute l2 distance of embedding to the closest centroid (computed_cluster_labels_cosine = id of closest centroid by cosine distance [0, ..., 99])

        start = time.time()
        labels_pred = np.zeros(len(features_cosine))
        for i in np.unique(computed_cluster_labels_cosine):
            # for each cluster id
            index = np.where(computed_cluster_labels_cosine == i)[0] # to which samples is it the closest centroid
            ind = np.argmin(d[index]) # smallest distance to the centroid of all images belonging to that centroid
            cid = index[ind] # the image id that is closest to the centroid (i.e. the prototype)
            labels_pred[index] = cid # Assign all images of this cluster to the prototype
        ...
        count_cluster = np.zeros(n_labels) # how many ground truth label per existing ground truth label (same order as avail_labels)
        for i in range(n_labels):
            count_cluster[i] = len(np.where(target_labels == avail_labels[i])[0])

        # build a mapping from item_id to item index
        keys     = np.unique(labels_pred) # all class prototypes
        num_item = len(keys)
        values   = range(num_item)
        item_map = dict()
        for i in range(len(keys)):
            item_map.update([(keys[i], values[i])]) # we map the first class prototype to the value 0 etc
        ...
        for i in range(N):
            ...
            count_item[index] = count_item[index] + 1 # count how many images are attributed to each class prototype
