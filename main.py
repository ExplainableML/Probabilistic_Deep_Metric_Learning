...
parser = par.speed_parameters(parser)
##### Read in parameters
...
"""============================================================================"""
#################### WARMSTART PROXIES ######################
def embed_data(model, dataloader, device):
    with torch.no_grad():
        features = []
        target_labels = []
        final_iter = tqdm(dataloader, desc='Warmstarting Proxies...')
        for idx, inp in enumerate(final_iter):
            input_img, target = inp[1], inp[0]
            target_labels.extend(target)
            out = model(input_img.to(device))
            if isinstance(out, tuple): out, aux_f = out
            features.extend(out.cpu().detach())
...
if "proxyvmf" in opt.loss and opt.loss_proxyvmf_warmstart:
    # Initialize proxies
    _ = model.eval()
    embeds, targets = embed_data(model, dataloaders['evaluation'], opt.device)
else:
    embeds, targets = None, None
...
criterion, to_optim = criteria.select(opt.loss, opt, to_optim, batchminer, embeds, targets)
...
for epoch in range(opt.n_epochs):
    ...
    # Report metrics for vMF
    if 'proxyvmf' in opt.loss:
        print("Avg proxy kappa: {}".format(torch.mean(criterion.proxies.weight_g.detach()).cpu().numpy()))
        print("Learned temperature: {}".format(criterion.temp.detach().cpu().numpy()))
